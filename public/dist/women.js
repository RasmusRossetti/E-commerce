const productList = document.getElementById("products-print");
...
searchBar.addEventListener("keyup", (e) => {
    ...
    const filteredCharacters = data.products.filter((products) => {
        return (
...
const loadCharacters = async() => {
    try {
        const res = await fetch("productwomen.json");
        data = await res.json();
        displayCharacters(data.products);
    } catch (err) {
        console.error(err);
    }
};
